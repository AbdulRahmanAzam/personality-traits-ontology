...
export function Results({ assessmentData, questions }) {
  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto p-6 min-h-screen flex flex-col gap-6 bg-linear-to-b from-slate-50 to-slate-100">
        <div className="flex flex-col items-center justify-center flex-1 py-16">
          <div className="w-12 h-12 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin mb-6"></div>
          <h2 className="text-2xl font-bold text-slate-800 mb-3">Analyzing your responses...</h2>
          <p className="text-slate-600">Calculating your personality profile using ontology data</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 min-h-screen flex flex-col gap-6 bg-linear-to-b from-slate-50 to-slate-100">
        <div className="flex flex-col items-center justify-center flex-1 py-16 text-center">
          ...
          <p className="text-slate-500">Please make sure the backend API is running.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 min-h-screen flex flex-col gap-6 bg-linear-to-b from-slate-50 to-slate-100">
      <div className="text-center py-8">
        <h1 className="text-3xl font-bold text-slate-800 mb-2">🎯 Your Personality Profile</h1>
        <p className="text-slate-600">Big Five Assessment Results (Powered by OWL Ontology)</p>
      </div>

      <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100 flex flex-col md:flex-row justify-between gap-6">
        <div className="flex flex-wrap gap-6">
          <div className="flex flex-col">
            <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Participant</span>
            <span className="text-lg font-semibold text-slate-800">{userData.name}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Age</span>
            <span className="text-lg font-semibold text-slate-800">{userData.age}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Country</span>
            <span className="text-lg font-semibold text-slate-800">{userData.country}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">University</span>
            <span className="text-lg font-semibold text-slate-800">{userData.university || 'N/A'}</span>
          </div>
        </div>
        <div className="flex flex-wrap gap-6">
          <div className="flex flex-col items-center bg-slate-50 px-4 py-2 rounded-xl">
            <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Total Time</span>
            <span className="text-lg font-bold text-cyan-600">{formatDuration(timestamps.totalDuration)}</span>
          </div>
          <div className="flex flex-col items-center bg-slate-50 px-4 py-2 rounded-xl">
            <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Avg. per Question</span>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100">
        <h2 className="text-2xl font-bold text-slate-800 mb-6"> Your Trait Analysis</h2>
        <div className="space-y-6">
          {traitScores.map(trait => (
            <div key={trait.key} className="border-b border-slate-100 pb-5 last:border-b-0 last:pb-0">
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-bold text-slate-800 text-lg">{trait.name}</h3>
                  ...
                  <div className="flex gap-4 text-sm">
                    <span className="text-slate-500">T-Score: <strong className="text-slate-700">{trait.tScore.toFixed(1)}</strong></span>
                    <span className="text-slate-500">Percentile: <strong style={{ color: trait.color }}>{trait.percentile.toFixed(1)}%</strong></span>
                    <span className="text-slate-500">Raw: <strong className="text-slate-700">{trait.rawScore}/{trait.maxScore}</strong></span>
                  </div>
                </div>
              </div>
              ...
              <p className="text-slate-600 leading-relaxed">{trait.description}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {apiResults?.predictions && Object.keys(apiResults.predictions).length > 0 && (
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100">
          <h2 className="text-2xl font-bold text-slate-800 mb-2">📈 Predicted Outcomes</h2>
          <p className="text-slate-500 text-sm mb-6">Based on meta-analytic correlations from psychological research</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(apiResults.predictions).map(([key, prediction]) => (
              <div key={key} className="bg-linear-to-br from-slate-50 to-slate-100 rounded-xl p-4 border border-slate-200">
                <h4 className="font-bold text-slate-800 mb-3">{formatPredictionName(key)}</h4>
                <div className="mb-3">
                ...
                <span className="text-sm text-slate-600 font-medium">{prediction.interpretation}</span>
                <div className="mt-3 flex flex-wrap gap-1">
                  {prediction.contributingTraits?.map((ct, idx) => (
                    <span key={idx} className="text-xs bg-slate-200 text-slate-600 px-2 py-1 rounded-full">
      ...
      { /* Timestamps Section */}
      <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100">
        <h2 className="text-2xl font-bold text-slate-800 mb-2">⏱️ Response Timing Details</h2>
        ...
        <div className="grid grid-cols-5 md:grid-cols-10 gap-2">
          {Object.entries(timestamps.questionTimestamps)
            ...
            .map(([id, data]) => (
              <div key={id} className="bg-slate-50 rounded-lg p-2 text-center">
                <span className="text-xs text-slate-500 block">Q{id}</span>
        ...
        {Object.keys(timestamps.questionTimestamps).length > 10 && (
          <button 
            ...
            className="mt-4 w-full py-2 px-4 bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
          >
            {showAllTimestamps ? (
              <>
                <span>Show Less</span>
